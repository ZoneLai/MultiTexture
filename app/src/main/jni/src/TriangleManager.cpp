...
void TriangleManager::initGL(int widgetWidth, int widgetHeight) {
    ...
    if (CompileShaderProgram(camera_play_vert, camera_play_frag, &_sProgramPlay)) {
        ...
        _sampler2DLoc1      = glGetUniformLocation(_sProgramPlay,   "u_Texture1");
...
void TriangleManager::drawFrame() {
    ...
    glBindTexture(GL_TEXTURE_2D, _textures[1]);
    glUniform1i(_sampler2DLoc, 1);      // 设置纹理单元为0号

    glUniform1f(_mixValueLoc, 0.4);
...
GLint TriangleManager::getTextureId() {
    return _textures[1];
}